...
protocol MediaPickerUseCase {
    func combineMovies(assets: [PHAsset], completion: @escaping (Result<URL?, Error>) -> Void)
...
final class DefaultMediaPickerUseCase: MediaPickerUseCase {
    ...
    func combineMovies(assets: [PHAsset], completion: @escaping (Result<URL?, Error>) -> Void) {
        var movies = [AVAsset]()
        self.requestAsset(from: assets[0]) { asset, audioMix, keyValue in
            if let asset = asset {
                movies.append(asset)
                self.requestAsset(from: assets[1]) { asset, audioMix, keyValue in
                    if let asset = asset {
                        movies.append(asset)
                        self.movieCombineEditor.combineMovies(assets: movies) { result in
                            switch result {
...
extension DefaultMediaPickerUseCase {
    ...
    private func requestAsset(from asset: PHAsset, resultHandler: @escaping (AVAsset?, AVAudioMix?, [AnyHashable : Any]?) -> Void) {
        self.mediaPickerRepository.requestAssetVideoWithDefaultOptions(of: asset, resultHandler: resultHandler)
    }
